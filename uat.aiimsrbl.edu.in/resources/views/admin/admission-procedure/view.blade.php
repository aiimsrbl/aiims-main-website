						...
						<table class="table table-bordered">
							<tbody>
								...
								<tr>
									...
									<td>
										@if($viewObj->file && file_exists(storage_path().'/app/public/admission-procedure/'.$viewObj->file))
												<a href="{{ route('image.displayImage',['p'=>'admission-procedure/'.$viewObj->file]) }}" target="_blank">
													<button type="button" class='btn btn-sm my-2 mx-2 btn-info'>Click To View File</button>
												</a>
										@endif
									</td>
								</tr>
								<tr><td><button onclick="redirect_url($(this),'{{route('admin.admission.procedure.listing')}}')"  type="button" class="btn btn-danger waves-effect waves-light">Cancel</button></td></tr>
							</tbody>
						</table>
					</div>
				</div>
