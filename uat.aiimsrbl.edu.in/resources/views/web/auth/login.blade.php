                  ...
                  <div class="wrapper wrapper2 border">
                     <!--Login Form-->
                        <form  class="card-body" tabindex="500" method="post" name="login" id="adminLogin">
                           ...
                           <div class="captcha">
                              <input type="text" style="border:solid thin black" id="captcha" name="captcha" maxlength="16" required>
                              <label>Captcha</label>
                              <img src="{{env('CAPTCHA_PATH')}}?captcha={{$rcaptcha}}" id='captcha_image' alt="captcha" style="height: 30px; margin-top: 35px;">
                           </div>
...
@push('scripts')
   <script>
      ...
      $('document').ready(function(){
         ...
         $.ajax({
            method    :  "POST",
            url       :  "{{env('CAPTCHA_PATH')}}?type=create&captcha={{$rcaptcha}}",
            dataType  :  "json",
            data      :  {},
            success   :  function()
            {  
